 ...
 * processors associated to the current printing task are stopped quickly since the printing task
...
public class CreateMapPagesFailOnErrorProcessorTest extends AbstractMapfishSpringTest {
  ...
  /**
   * Slow file handler, that will fill fail on the first request
   *
   * @param filename
   * @return
   */
  protected TestHttpClientFactory.Handler createFailingFileHandler(Function<URI, String> filename) {
    return new TestHttpClientFactory.Handler() {
      ...
      public MockClientHttpRequest handleRequest(URI uri, HttpMethod httpMethod) throws Exception {
        wmsRequestsNb.incrementAndGet();
        if (wmsRequestsNb.get() == 40) {
  ...
  public void testExecute() throws Exception {
    ...
    testPrint(config, requestData, format, 70);
  }
